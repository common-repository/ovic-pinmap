<?php
if (!class_exists('Ovic_Pinmap_Shortcode')) {
    class Ovic_Pinmap_Shortcode
    {
        /**
         * Initialize.
         *
        ...
        public function __construct()
        {
            ...
            // Add shortcode
            add_shortcode('ovic_pinmap', array($this, 'render_shortcode'));
        }
        ...
        public function content_image($pin)
        {
            ...
            if ($pin['settings']['popup-title']) {
                $html .= '<div class="ovic-popup-title">';
        ...
        public function content_link($pin)
        {
            ...
            if ($pin['settings']['popup-title']) {
                ...
                $html .= '<h3 class="ovic-title">'.esc_html__('Add your title in backend', 'ovic-pinmap').'</h3>';
            }
        ...
        public function content_woocommerce($pin)
        {
            ...
            if (!empty($product)) {
                ...
                if ($pin['settings']['product-thumbnail']) :
                    $image_size = apply_filters('ovic_pinmap_product_thumbnail', array(
                        $pin['settings']['woo-width'],
                        $pin['settings']['woo-height']
                    ));
                    $thumbnail = $product->get_image($image_size);
                    $thumbnail = apply_filters('ovic_pinmap_product_thumbnail_html', $thumbnail, $product, $pin);
                    ?>
                    <div class="col-left product-thumbnail">
        ...
        public function render_shortcode($atts, $content = null)
        {
            ...
            // Get current image.
            $attachment_id = get_post_meta($atts['id'], 'ovic_pinmap_image', true);
            if (!$attachment_id) {
                return '';
            }
            // Get content source.
            ...
            $settings = get_post_meta($atts['id'], 'ovic_pinmap_settings', true);
            // Get all pins.
            $pins = get_post_meta($atts['id'], 'ovic_pinmap_pins', true);
            // Check is empty
            if (empty($pins)) {
                return '';
            }
            ...
            <div id="<?php echo esc_attr($id_shortcode); ?>" class="shortcode-pinmap-builder">
                <div <?php $this->attribute($pinmap_attr); ?>>
                    ...
                    <?php if ($settings['image-effect'] == 'mask'): ?>
                        <div class="mask"></div>
                    <?php endif; ?>
                    <?php if (!Ovic_Pinmap_Post_Type::is_elementor_editor()): ?>
                        <?php foreach ($pins as $index => $pin) :
                            $pin_attr = array(
                                'id'            => 'ovic-pin-'.$pin['settings']['id'],
                                'class'         => array(
                                    'ovic-pin',
                            ...
                            <div <?php $this->attribute($pin_attr); ?>>
                                ...
                                if (method_exists($this, $method)) {
                                    ...
                                    echo apply_filters('ovic_content_type_pinmap', $this->$method($pin), $atts, $this);
            ...
            return apply_filters('ovic_render_shortcode_pinmap', ob_get_clean(), $atts);
        }
        ...
        public function enqueue_scripts()
        {
            ...
            wp_register_style('ovic-tooltip', OVIC_PINMAP_URL.'assets/3rd-party/tooltip/tooltip.css');
            wp_register_script('ovic-tooltip', OVIC_PINMAP_URL.'assets/3rd-party/tooltip/tooltip.min.js', array(), false, true);

            wp_register_style('ovic-pinmap', OVIC_PINMAP_URL.'assets/css/pinmap.min.css', array('ovic-tooltip'), OVIC_PINMAP_VERSION);
            wp_register_script('ovic-pinmap', OVIC_PINMAP_URL.'assets/js/pinmap.min.js', array('ovic-tooltip'), OVIC_PINMAP_VERSION, true);
        }
        ...
        public function inline_style($id, $settings, $pins)
        {
            ...
            $post_custom_css .= get_post_meta($id, '_Ovic_VC_Shortcode_Custom_Css', true);
            // Generate CSS rules for general settings.
            $css .= '#ovic-pinmap-'.$id.' .ovic-popup {';
            $css .= 'padding: 10px;';
            ...
            if ($pins) {
                foreach ($pins as $pin) {
                    // Popup width & height
                    $css .= '
					#ovic-pinmap-'.$id.' #ovic-pin-'.esc_attr($pin['settings']['id']).' .ovic-popup {';
                    ...
                    // Pin style setting
                    $css .= '
					#ovic-pinmap-'.$id.' #ovic-pin-'.esc_attr($pin['settings']['id']).' .icon-pin {';
                    ...
                    // Pin hover setting
                    $css .= '
					#ovic-pinmap-'.$id.' #ovic-pin-'.esc_attr($pin['settings']['id']).' .icon-pin:hover {';
                    ...
                    if ($pin['settings']['icon-type'] == 'icon-area') {
                        $css .= '
					    #ovic-pinmap-'.$id.' #ovic-pin-'.esc_attr($pin['settings']['id']).' .text__area{';
                        ...
                        // Hover text
                        $css .= '
                	    #ovic-pinmap-'.$id.' #ovic-pin-'.esc_attr($pin['settings']['id']).' .text__area:hover,
                 	    #ovic-pinmap-'.$id.' #ovic-pin-'.esc_attr($pin['settings']['id']).' img:hover{ ';
                        $css .= '}';
                    }
                }
            }
