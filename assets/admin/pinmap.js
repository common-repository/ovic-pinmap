(function ($) {
    ...
    var initTab = function () {
        $(document).on('click', '.ovic-pinmap ul.nav li', function (e) {
    ...
    var imageSelector = function () {
        $(document).on('click', '.ovic-pinmap a.btn-add-image, .ovic-pinmap #change-image, .ovic-pinmap a.image-selector', function (event) {
            ...
            if (!window.wr_image_selector) {
                ...
                window.wr_image_selector = wp.media({
                    button: {
                        text: ovic_pinmap_params.text.img_selector_btn_label,
                    },
                    states: [new wp.media.controller.Library({
                        title   : ovic_pinmap_params.text.img_selector_modal_title,
                ...
                window.wr_image_selector.on('select', function () {
                    ...
                    if (window.wr_image_selector.input_element.attr('id') == 'ovic_pinmap_image') {
                        window.wr_image_selector.input_element.val(attachment.attributes.id);

                        // Update edit area with new image.
                        if ($('.ovic-pinmap-bot > .edit-image').length) {
                            $('.ovic-pinmap-bot .image-wrap > img').attr('src', attachment.attributes.url);
                        } else {
                            var edit_html = $('#ovic_pinmap_image_tmpl').text().replace('%URL%', attachment.attributes.url);

                            $('.ovic-pinmap-bot > .add-image').replaceWith(edit_html);
            ...
            window.wr_image_selector.input_element = $(this).hasClass('image-selector') ? $(this).prev('input') : $('#ovic_pinmap_image');
    ...
    var setupApplication = function () {
        ...
        var PinList = Backbone.Collection.extend({
            ...
            fetch: function () {
                if (window.ovic_pinmap_pins && window.ovic_pinmap_pins.length) {
                    this.add(window.ovic_pinmap_pins);
                }
            },
        });
        ...
        var PinView = Backbone.View.extend({
            ...
            template: _.template($('#ovic_pinmap_pin_tmpl').text()),
                ...
                this.$el.find('[data-option]').each(function () {
                    ...
                    if ($(this).attr('name') != option[0]) {
                        if (option[3] !== undefined) {
                            $(this).attr('name', 'ovic_pinmap_pins[' + self.index + '][' + option[1] + '][' + option[3] + ']');
                        } else {
                            $(this).attr('name', 'ovic_pinmap_pins[' + self.index + '][' + option[1] + ']');
                        }
                    }
                    ...
                    switch ($(this).attr('data-option')) {
                        ...
                        case 'settings[area-text-size]':
                            ...
                            self.$el.find('.icon-pin').next().next('div.text__area').removeClass('hidden');
                            break;
                        case 'settings[area-text-line-height]':
                            $(this).keyup(function () {
                                if ($(this).val() != '' && 'icon-area' == self.$el.find('[data-option*="icon-type"]:checked').val()) {
                    ...
                    if ($(this).hasClass('color-picker')) {
                        $(this).ovicColorPicker();

                        setTimeout($.proxy(function () {
                            $(this).parent().parent().click(function () {
                                // Hide all icon selector popup if not being
                                // focused.
                                $('.ovic-pinmap .icon-selector').each(function () {
                    ...
                    // Init icon selector if needed.
                        ...
                        $(this).addClass('hidden').after($('#ovic_pinmap_icon_selector_tmpl').text().replace('%SELECTED%', $(this).val()));
                    }
                    ...
                    // Init product selector if needed.
                        ...
                        $(this).select2({
                            ...
                            ajax              : {
                                url     : ovic_pinmap_params.product_selector.url,
                                dataType: 'json',
                                delay   : 250,
                                data    : function (terms) {
                                    return {
                                        term    : terms,
                                        security: ovic_pinmap_params.product_selector.security,
                                    };
                                },
                            ...
                            initSelection     : function (element, callback) {
                                ...
                                if (id !== '') {
                                    $.ajax(ovic_pinmap_params.product_selector.url + '&term=' + id + '&security=' + ovic_pinmap_params.product_selector.security, {
                                        dataType: 'json',
                                    }).done(function (data) {
                                        if (data !== null)
                                            callback({
            ...
            edit: function (event) {
                ...
                if (this.$el.hasClass('editing')) {
                    ...
                    if (!this.$el.data('ovic_pinmap_pin_settings_initialized')) {
                        var self = this;
                        ...
                        this.$el.data('ovic_pinmap_pin_settings_initialized', true);
                    }
            ...
            close: function (event) {
                ...
                if (pin_type == 'woocommerce') {
                    ...
                    this.$el.find('.tooltip').text(title ? title : ovic_pinmap_params.text.please_input_a_title).removeClass('hidden');
                }
            ...
            remove: function (event) {
                ...
                if (confirm(ovic_pinmap_params.text.confirm_removing_pin)) {
                    this.$el.remove();
                    this.model.destroy();
        ...
        var PinListView = Backbone.View.extend({
            ...
            el: '.ovic-pinmap-bot > .edit-image > .image-wrap',
            ...
            initialize: function () {
                ...
                $(document).on('click', '.ovic-pinmap-bot > .edit-image > .image-wrap > img', this.create);
        ...
        $(document).on('init_ovic_pinmap', function (event) {
            if (!$(document).data('ovic_pinmap_settings_initialized')) {
                ...
                $('#general-settings').find('input, select, textarea').each(function () {
                    if (window.ovic_pinmap_settings[$(this).attr('name')]) {
                        $(this).val(window.ovic_pinmap_settings[$(this).attr('name')]);

                        if ($(this).attr('type') == 'hidden' && $(this).next().attr('type') == 'checkbox') {
                            if (parseInt(window.ovic_pinmap_settings[$(this).attr('name')])) {
                                $(this).next().attr('checked', 'checked');
                            } else {
                                $(this).next().removeAttr('checked');
                            }
                    ...
                    $(this).attr('name', 'ovic_pinmap_settings[' + $(this).attr('name') + ']');
                ...
                $('#general-settings').on('change', 'input[type="radio"], select', function () {
                    var option = $(this).attr('name').match(/ovic_pinmap_settings\[([^\]]+)\]/),
                        value  = $(this).val();
                ...
                $(document).click(function (event) {
                    ...
                    $('.ovic-pinmap .wp-picker-holder .iris-picker').each(function () {
                ...
                // Track data changes.
                var data_fields = '.ovic-pinmap input, .ovic-pinmap select, .ovic-pinmap textarea';

                $(document).on('change', data_fields, function () {
                    if ($(this).attr('name') && $(this).attr('name').indexOf('ovic_pinmap_') > -1) {
                        $('#post #publish').attr('data-changed', 'yes');
                    }
                });
                ...
                $(window).on('beforeunload', function () {
                    ...
                    if (submit_btn.length && submit_btn.attr('data-changed') == 'yes') {
                        return ovic_pinmap_params.text.ask_for_saving_changes;
                    }
                });

                $(document).data('ovic_pinmap_settings_initialized', true);
            }

            if (!window.ovic_pinmap_pins_app && $('.ovic-pinmap-bot > .edit-image > .image-wrap').length) {
                // Init pin list view.
                window.ovic_pinmap_pins_app = new PinListView;
            }
        });
    };
